...
public class GatheringService {
	...
	public PageResponse<GatheringResponseDto> findAllGathering(GatheringSearchRequestDto gatheringSearchRequestDto,
		Pageable pageable) {
		return PageResponse.from(
			gatheringRepository.searchByCriteria(gatheringSearchRequestDto, pageable).map(GatheringResponseDto::from));
	}

	public PageResponse<GatheringResponseDto> findGathering(GatheringStatus gatheringStatus, AppTechPlatform platform,
		Pageable page) {
	...
	public PageResponse<GatheringResponseDto> findMyGathering(Long memberId, Boolean isActive, String made,
		Pageable page) {
	...
	public GatheringMakeResponseDto makeGathering(Long memberId, GatheringMakeRequestDto gatheringMakeRequestDto) {
		...
		Gathering gathering = Gathering.builder()
			...
			.gatheringImageUrl(
				gatheringMakeRequestDto.gatheringImageUrl() == null ? "" : gatheringMakeRequestDto.gatheringImageUrl())
			...
			.startDate(
				gatheringMakeRequestDto.startDate() == null ? LocalDateTime.now() : gatheringMakeRequestDto.startDate())
			.endDate(gatheringMakeRequestDto.endDate() == null
				? LocalDateTime.now().plusDays(gatheringMakeRequestDto.workingDays())
				: gatheringMakeRequestDto.endDate())
	...
	public GatheringStatisticsResponseDto gatheringStatistics(Long memberId) {
		...
		Long pendingCount = gatheringMemberRepository.countByMemberIdAndGathering_Status(memberId,
			GatheringStatus.PENDING);
		Long onGoingCount = gatheringMemberRepository.countByMemberIdAndGathering_Status(memberId,
			GatheringStatus.ONGOING);
		Long completedCount = gatheringMemberRepository.countByMemberIdAndGathering_Status(memberId,
